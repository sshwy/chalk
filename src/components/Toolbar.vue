  ...
  <div class="pointer-events-none fixed top-4 inset-x-0 flex justify-center z-10">
    <div
      ...
      :class="props.mode === 'dark'
        ...
        ">
      <ToolButton :mode="props.mode" :selected="props.tool === 'drag'" variant="sky" aria-label="Drag mode"
        @click="setTool('drag')">
        <Hand class="w-4 h-4" aria-hidden />
      </ToolButton>

      <div class="relative inline-flex">
        <ToolButton :mode="props.mode" :selected="props.tool === 'pen'" variant="sky" aria-label="Pen mode"
          @click="handlePenClick">
          <Pencil class="w-4 h-4" aria-hidden />
        </ToolButton>
        <div v-if="showPenWidth && props.tool === 'pen'"
          ...
          <div class="flex flex-row items-center gap-3">
            <button v-for="w in PEN_WIDTH_OPTIONS" :key="w" type="button"
              ...
              :aria-label="`线宽 ${w}`" @click="setPenWidth(w)" />
          </div>
        </div>
      </div>

      <div class="relative inline-flex">
        <ToolButton :mode="props.mode" :selected="props.tool === 'brush'" variant="sky"
          aria-label="Brush (erase strokes)" @click="handleBrushClick">
          <Eraser class="w-4 h-4" aria-hidden />
        </ToolButton>
        <div v-if="showBrushRadius && props.tool === 'brush'"
          class="absolute left-1/2 top-full mt-4 -translate-x-1/2 rounded-lg border px-3 py-3 shadow-lg"
          :class="props.mode === 'dark' ? 'bg-slate-800 border-white/20' : 'bg-white border-slate-200'">
          <div class="flex flex-row items-center gap-3">
            <button v-for="size in BRUSH_SIZE_OPTIONS" :key="size" type="button"
              ...
              :aria-label="`半径 ${size}`" @click="setBrushRadius(size)" />
          </div>
        </div>
      </div>

      <ToolButton :mode="props.mode" :disabled="!props.canUndo" aria-label="Undo" @click="handleUndoClick">
        <Undo2 class="w-4 h-4" aria-hidden />
      </ToolButton>

      <ToolButton :mode="props.mode" :disabled="!props.canRedo" aria-label="Redo" @click="handleRedoClick">
        <Redo2 class="w-4 h-4" aria-hidden />
      </ToolButton>

      <ToolButton v-if="isTouchScreen" :mode="props.mode" :selected="props.penOnly" variant="violet"
      ...
      <ToolButton :mode="props.mode" :selected="props.mode === 'dark'" variant="amber" aria-label="Toggle theme"
        @click="toggleMode">
